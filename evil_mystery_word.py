...
def update_after_guess(character, string_list, number):
    branch_list = []
    for position in range(len(string_list[0])):
        for item in range(len(string_list)):
            if character == string_list[item][position]:
                branch_list.append([position, string_list[item]])
            else:
                branch_list.append([-1, string_list[item])
    for number in range(len(string_list[0])):
        

    for index in range(len(string_list[0])):
        if character in string_list[0]
...
def select_mystery_word_list(list_of_two_numbers):
    list_of_choices = []
    length_number = random.int(range(list_of_two_numbers[0]), list_of_two_numbers[1]+1)
    with open("/usr/share/dict/words") as f:
        for word in f:
            word = word.strip()
            if len(word) == length_number:
                list_of_choices.append(word)
    return list_of_choices.upper()
...
def main():
    ...
    mystery_word_list = select_mystery_word_list(
    ...
    output_word_size(mystery_word_list[0]) #no return; print word length once

    update_game_state(mystery_word_list[0], guessed_letters) #no return; print game 'board'
    ...
    while True:
        ...
        return_boolean, guess_count = update_after_guess(guess, mystery_word_list, guess_count) #TESTABLE
